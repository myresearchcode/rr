...
def stratified_recall(numr,denom):
    ...
    return recall;
#########################################################################################
def get_user_metrics(user_metric_dict,U_rmd,U_curv,u_ind,k,R_train,R_test,Mov_Gen_Mat,M,M_map,m_ind,V):
    rmd = U_rmd[u_ind][:k]
    ...
    user_metric_dict[k]['srn'][u_ind] = strat_recall_numr_per_user(R_test,u_ind,rmd)
    user_metric_dict[k]['gc'][u_ind] = true_topic_coverage(R_test,u_ind,rmd,M,M_map,4)
    user_metric_dict[k]['ild'][u_ind] = ild(Mov_Gen_Mat,rmd)
    ...
    user_metric_dict[k]['serendipity_score'][u_ind] = serendipity_score(R_test,u_ind,rmd)
